                    ...
                    {layout === "closed_ring" ? (
                      <div className="relative mx-auto mt-6 h-[280px] w-full max-w-[340px]">
                        {normalizedBondOrders.map((order, index) => {
                          ...
                          const endAngle =
                            ((Math.PI * 2) / activeCarbonCount)
                              * ((index + 1) % activeCarbonCount) - Math.PI / 2;
                          const x1 = 50 + Math.cos(startAngle) * 32;
                          const y1 = 50 + Math.sin(startAngle) * 32;
                          const x2 = 50 + Math.cos(endAngle) * 32;
                          const y2 = 50 + Math.sin(endAngle) * 32;
                          ...
                          return (
                            <div
                              ...
                              style={{
                                width: `${distance}%`,
                                transform: `translate(${x1 - 50}%, ${y1 - 50}%) rotate(${angle}deg)`,
                                transformOrigin: "0 50%",
                              }}
                            >
                              <div className="relative h-4">
                                <div className="absolute top-1/2 h-px w-full -translate-y-1/2 bg-amber-200/70" />
                                {order === 2 ? (
                                  <div className="absolute top-[calc(50%+5px)] h-px w-full -translate-y-1/2 bg-amber-200/70" />
                                ) : null}
                              </div>
                            </div>
                          );
                              ...
                              <div className="flex flex-col items-center gap-2">
                                <div className="flex min-h-6 items-center justify-center gap-1">
                                  {Array.from(
                                    ...
                                    (_, hydrogenIndex) => (
                                      <div
                                        key={`ring-top-h-${index}-${hydrogenIndex}`}
