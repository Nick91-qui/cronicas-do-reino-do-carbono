    ...
    <main className="mx-auto w-full max-w-6xl px-6 py-10">
      ...
      <section className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {collection.molecules.length > 0 ? collection.molecules.map((molecule) => (
          <div key={molecule.id} className="rounded-[28px] border border-white/8 bg-white/5 p-2 shadow-[0_18px_40px_rgba(15,23,42,0.18)]">
            <MoleculeCard
              molecule={molecule}
              variant="compact"
            />
          ...
          <article className="rounded-[28px] border border-dashed border-white/10 bg-white/5 p-8 text-sm text-slate-300">
